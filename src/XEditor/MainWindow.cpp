...
void MainWindow::onTriggeredConnect(bool)
{
    ...
    pDialogConnect->show();
    m_wndNetConfig.LoadConfig();
    if( IDOK != m_wndNetConfig.DoModal() )
        return;
    if( m_bConnectStatus )OnOptDisconn();
    if (!CElementMgr::Setup("Element",m_wndNetConfig.m_sAddress.GetBuffer(),m_wndNetConfig.m_nPort)){
        //AfxMessageBox(_T("连结失败!"));
        return;
    }
    m_bConnectStatus = TRUE;
    m_wndClassView.FillClassView();
    m_wndClassView.UpdateRootText(m_wndNetConfig.m_sAddress);
    m_wndClassView.SendMessage(WM_COMMAND,ID_CLASS_COLLAPSE,0);
    UpdateStatusBar();
}
//断开
void MainWindow::onTriggeredDiscon(bool)
{
    ...
    CElementMgr::Destory();
    CXEditorView* pView = (CXEditorView *)GetActiveView();
...
void MainWindow::onTriggeredResetService(bool)
{
    ...
    // TODO: 在此添加命令处理程序代码
    std::string strOut;
    CElementMgr::SendCmd("./start.sh", strOut);
    std::string strVersion;
    int nVersion;
    nVersion = CElementMgr::GetVersion(strVersion);
    MessageBox(strOut.c_str(), "返回");
}
//复位设备
...
{
    qDebug()<<"新增";
        pCmdUI->Enable(m_wndProperties.m_pElement && !m_wndProperties.m_bIsBranch);
}
//修改
void MainWindow::onTreggeredModify(bool)
{
    qDebug()<<"修改";
        pCmdUI->Enable(m_bConnectStatus && m_wndProperties.m_pElement && !m_wndProperties.m_bIsParent);
}
//剪切
